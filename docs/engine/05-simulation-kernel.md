...
(북극성: 「둥지짓는 드래곤」의 시스템 구조를 차용한 새 게임). 그 게임의 심장인 **일(日) 단위 턴 루프**의
...
> **현재 범위(첫 슬라이스)**: `커맨드 선택 → 자원 증감 → 다음 일차`. 클램프/최소값/파산·승패 판정, 효과 수식화,
...
- **순수 함수형 턴**: `ExecuteCommand`는 상태를 변형하지 않고 **새 `RunState`를 반환**합니다(불변). 세이브·되감기·테스트에 유리.
...
### 초기 상태
```csharp
RunState CreateInitialState()
```
- `Day = 1`, 각 자원을 `StartValue`로 채운 상태를 반환.

### 커맨드 실행 (순수)
```csharp
RunState ExecuteCommand(RunState state, string commandId)
```
- 현재 자원을 복제한 뒤, 커맨드의 각 `ResourceDelta`를 **더함**(`값 += Amount`).
- **일차를 +1** 한 새 `RunState`를 반환(입력 state는 불변).
...
- `SimController` (MonoBehaviour):
  - 인스펙터의 SO 리스트 → def로 변환 → `TurnEngine` 구성(배선 오류면 여기서 콘솔 에러) → `LoopEngine`으로 감쌈.
  - 커맨드마다 버튼을 생성, 클릭 시 `LoopEngine.ExecuteCommand(campaign, commandId)` 실행(Run만 전진).
  - "새 회차" 버튼(있으면) 클릭 시 `LoopEngine.StartNewLoop(campaign)` 실행(회차 갱신).
  - 상태 라벨(`회차: N   일차: N   재보: X   마력: Y`)을 매 실행 후 갱신.
...
라이브 검증: 회차1/일차1(재보100/마력50) → 약탈 → 회차1/일차2(150/30) → 새 회차 → 회차2/일차1(100/50).
> "회차"는 [06 회차 루프](06-loop-and-state.md)에서 도입한 개념(현재는 얇은 버전 — `LoopCount+1` + Run 리셋뿐).
