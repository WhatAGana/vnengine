...
### 마이그레이션
1. 기존 `dialogues.json`의 요르 데이트 시나리오를 `intro.vns` 로 이식하고 `Assets/Resources/scripts/`에 둔다.
2. 씬 `Main`: `DialogueManager` 컴포넌트 → `VNRunner` 로 교체. `Canvas`·슬롯·`EventSystem` 재사용. `DialogueViewUnity`/`StageViewUnity` 를 씬 오브젝트에 배치·연결. `Canvas`의 `CanvasScaler`를 모바일 반응형으로 설정(7.3).
3. `DialogueManager.cs` 는 파리티 확인 전까지 **남겨둔다**. 확인 후 삭제.
4. `StreamingAssets/dialogues.json` 은 파리티 확인 후 제거.
...
5. **모바일 안전 로딩**: 스크립트 로딩이 `File.IO`/폴더 열거에 의존하지 않고 `Resources.LoadAll<TextAsset>`로만 동작(Android/iOS 호환).
...
## 7. 모바일 대응 (2026-07-04 추가)

**타깃 플랫폼**: Android + iOS + Windows/Mac 데스크톱. 코어(파서·컴파일러·인터프리터·표현식)는 순수 C#이라 플랫폼 무관 — 영향은 Unity 레이어에만 있다.

### 7.1 스크립트 로딩 — Resources + `.vns` ScriptedImporter
**문제**: 안드로이드는 `StreamingAssets`가 APK(jar) 안에 압축돼 있어 `System.IO.File`/`Directory.GetFiles`로 읽거나 열거할 수 없다(WebGL도 동일). 초기 계획의 `Directory.GetFiles` 로더는 모바일에서 동작하지 않는다.

**결정**: `.vns` 파일을 **커스텀 ScriptedImporter**로 `TextAsset`으로 임포트하고 `Assets/Resources/scripts/`에 두어, 런타임에 `Resources.LoadAll<TextAsset>("scripts")`로 로드한다.
- 에디터·PC·Android·iOS·WebGL 전부 **동일하게 동작**하고 런타임 열거가 가능하다(빌드에 포함된 자산이라 파일시스템 열거 불필요).
- `.vns` 확장자와 작가 워크플로를 유지한다(ScriptedImporter가 `.vns`→`TextAsset` 변환).
- **동기 로딩**이라 UnityWebRequest 코루틴이 불필요. 스크립트는 텍스트라 빌드 부담 미미.
- 파일 라벨(에러 메시지용)은 `TextAsset.name`.

이 결정으로 `StreamingAssets`는 P0에서 스크립트 로딩에 쓰지 않는다.

### 7.2 입력 — 탭 + 마우스
Unity 레거시 입력에서 첫 번째 터치는 `Input.GetMouseButtonDown(0)`으로 매핑되므로, 화면 클릭-진행 로직이 **터치에서 그대로 동작**한다. 별도 터치 코드 불필요. `EventSystem.IsPointerOverGameObject()`가 터치/마우스 모두에서 UI 위 입력을 구분한다.

### 7.3 반응형 UI
- `Canvas`의 `CanvasScaler`를 **Scale With Screen Size**로, 기준 해상도 1920×1080, Match 0.5로 설정해 다양한 화면비에서 대사 UI가 읽히게 한다.
- 노치/세이프에어리어 대응(SafeAreaFitter)은 **P1 폴리시**로 미룬다(P0는 기본 CanvasScaler까지).

### 7.4 확정된 설계 결정 (구현 계획)
- 스크립트 위치/로딩: **Resources + `.vns` ScriptedImporter** (위 7.1).
- 인터프리터 구동: **Unity 코루틴** 호스트(`VNRunner`)가 프레임마다 `Tick()`.
- 진입 라벨: `VNRunner` 인스펙터 필드, 기본 `start`.

---

## 8. 범위 밖 — 모바일 관련
- 세이프에어리어/노치 대응, 화면 회전 대응, 레터박스 → **P1**.
- Addressables/원격 스크립트 업데이트(재빌드 없이 교체) → 필요 시 나중에 전환(현재 Resources 로더를 대체 가능).
